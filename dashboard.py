...
# KONFIGURASI
# ─────────────────────────────────────────────
LIVE_MODE = True  # Mode SSE streaming
SSE_URL   = "http://localhost:8080/data/realtime"  # Endpoint SSE dari auto_generate.py
...
def _sse_listener(packet_queue: queue.Queue, running_flag: list):
            ...
            for line in r.iter_lines():
                ...
                if line:
                    ...
                    if s.startswith("data: "):
                        try:
                            raw_data = json.loads(s[6:])
                            
                            # Cek apakah data sudah terenkripsi atau belum
                            if "encrypted_payload" in raw_data:
                                # Data terenkripsi, dekripsi dulu
                                packet = {
                                    "encrypted_payload": raw_data["encrypted_payload"],
                                    "encrypted_aes_key": raw_data["encrypted_aes_key"],
                                    "nonce": raw_data["nonce"],
                                }
                                payload = unpack_packet(packet)
                            else:
                                # Data plain dari auto_generate.py, enkripsi lalu dekripsi untuk konsistensi
                            ...
                            if "label_name" in payload:
                                ...
                                # Dari auto_generate.py
                                label_val = int(payload.get("label", 0))
                                payload["label_name"] = LABEL_MAP.get(label_val, "NORMAL")
                                payload["voting_prediction"] = label_val
...
def start_sse_listener():
    ...
    if thread is None or not thread.is_alive():
        running_flag = [True]
        st.session_state.sse_running_flag = running_flag
        t = threading.Thread(
            target=_sse_listener,
            args=(st.session_state.packet_queue, running_flag),
            daemon=True
        )
        t.start()
...
def init_state():
    ...
    if "packet_queue" not in st.session_state:
        st.session_state.packet_queue = queue.Queue(maxsize=200)
...
def main():
    ...
    with st.sidebar:
        ...
        if LIVE_MODE:
            sse_thread = st.session_state.get("sse_thread")
            sse_ok = sse_thread is not None and sse_thread.is_alive()
            if sse_ok:
    ...
    # Auto-refresh logic untuk streaming
